...
zori_raw = read_csv("./data/Zip_zori_uc_sfrcondomfr_sm_month_NYC.csv", guess_max = 20000)
cat("ZORI raw column names (first 30):\n")
...
# check
cat("Final dataset rows:", nrow(zori_final), "\n")
cat("Unique ZIPs:", length(unique(zori_final$zip)), "\n")
...
cat("Total observations:", nrow(zori_final), "\n")
cat("Unique ZIP codes:", length(unique(zori_final$zip)), "\n")
cat("Unique neighborhoods:", length(unique(na.omit(zori_final$neighborhood))), "\n")

# --- Identify ZIPs in lookup but not in Zillow ZORI data ---
missing_zips = setdiff(zip_lookup$zip, zori_final$zip)
cat("Number of ZIP codes in lookup but missing from Zillow data:", length(missing_zips), "\n")
head(missing_zips, 10)  # show a few examples

# --- Compare rental prices Jan 2020 vs Jan 2021 ---
